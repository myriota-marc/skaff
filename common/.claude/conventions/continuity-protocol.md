...
- SessionStart startup|resume|clear|compact -> session-start.sh: <= 40 lines, highest value first so the cap only trims history: branch, stale docs, open gates, STATE front matter without `verified` and `status: done` items, last 10 ADR index lines, git log --oneline -5.
...
- PreToolUse Bash|PowerShell -> guard-bash.sh (both tools pass tool_input.command): blocks git commit on main/master in the repo the command targets (`git -C <dir>`, else a preceding `cd`/`Set-Location <dir>`, relative paths resolved against the hook's cwd, else the project repo); --no-verify; force push; commit --amend of a pushed commit in that same repo; shell writes to .gates/ or docs/decisions/ other than via scripts/. Write detection is quote-aware and looks only at real targets (after > or >>, tee/rm/mv/touch/truncate args, cp/ln destination, dd of=, sed -i files, git rm/mv paths), so editing docs/STATE.md to add `.gates/A1.json` to `verified` passes. The branch is read before the command runs: switch branches in one Bash call, commit in the next.
...
## Selftest
scripts/selftest.sh in throwaway worktree on temp branch; feeds hook scripts sample JSON; prints case|expected|actual; non-zero on mismatch. Cases: edit ADR 0001; docs without type; em dash; passive sentence; bad commit subject; commit on main; --no-verify; Closes-Item without evidence; stale doc at SessionStart; Stop with open item; clean commit passes; STATE edit mentioning .gates passes; shell write into .gates blocks; switch and commit in one call on main blocks; PowerShell tool is guarded; git -C commit in another repo on a branch passes; git -C commit in another repo on main blocks.; a grown STATE (30 items, mostly done) still shows the STALE line and every open item within 40 lines. A repo with no ADR gets a staged fixture ADR for case 1. The self-test is the gate for any hook change (item A1).
...
5. Claude Code picks up project .claude/settings.json hooks mid-session (confirmed on 2.1.281: guard-write blocked a live Write). guard-bash matches the hook bypass flag as literal text, so a command that merely mentions it (in a commit message or doc edit) is refused; write the text with the Write tool.
...
8. guard-bash reads the branch before the command runs, so `git switch -c topic && git commit ...` on main is blocked. Switch in one call, commit in the next.
9. Before the narrowed write check, any `sed -i` or `>` command that mentioned `.gates/` was blocked, even when it wrote docs/STATE.md. Self-test cases 12 and 13 pin the new behaviour.
10. /do-work-run needs the `bladnman/do-work` skill (verify-request, verify-plan, cleanup actions), which Skaff does not install. Install it with `npx skills add bladnman/do-work`, or use the stand-in procedure in INSTALL.md.
11. Long specialist runs (implement, git-workflow) hit 30 to 50 turn limits. Resume the same agent with SendMessage instead of spawning a fresh one, which would lose its context.
12. Never route around a hook: no second shell tool (PowerShell instead of Bash), wrapper script, `sh -c`, or rephrased command to get past a block. Stop and report the block to the human or the orchestrator. If the hook is wrong, fix the hook (with a self-test case) in its own change. guard-bash matches both Bash and PowerShell for this reason; existing installs whose settings.json registers it for `Bash` only need the matcher changed to `Bash|PowerShell` by hand, since the installer merge skips hook commands that are already present.
13. A hook scoped to the project repo misjudges commands aimed at other repos. guard-bash resolves the target repo from `git -C` or a leading `cd` before checking the branch.
14. A fixed line cap cuts whatever prints last. Once STATE grew, the 40-line SessionStart cap dropped the stale-docs section and self-test case 9 failed. Print the highest-value sections first, omit done items and the verified list, and keep self-test case 18 (grown STATE) as the guard.
15. Offline gates can pass while the platform call contract is wrong. Test handlers the way the platform invokes them (its payload shape, entry point and return contract), not only through your own wrapper.
16. Generator output must pass the target platform's own linter as a gate, not just local tests.

## Opt-in guards
`continuity/examples/guard-writes.sh` in the Skaff repo (not installed by default) is a PreToolUse hook that returns `permissionDecision: "ask"` for writes to external systems (REST hosts, CLI deploys, MCP write tools) and lets reads pass. Copy it to .claude/hooks/, edit its four patterns, record the choice in an ADR, and register it for the Bash matcher and each guarded MCP matcher (e.g. `mcp__.*[Aa]tlassian.*`) in .claude/settings.json.
